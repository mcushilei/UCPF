...
static const uint8_t port0PinRegIndexTable[29] = {
    17, 11,  6,  5,
     4,  3, 16, 15,
    14, 13,  8,  7,
     2,  1, 18, 10,
        ...
        switch (pConfig->Port) {
            case 0:
                if (pConfig->PIN >= UBOUND(port0PinRegIndexTable)) {
                    ...
                    uint8_t index = port0PinRegIndexTable[pConfig->PIN];
                    ...
                    if (fun >= PIN_FN_ACMP_I1) {
                        ...
                        uint8_t n = pConfig->Function >> 2;
                        uint32_t shift = 8 * (pConfig->Function & 0x03u);
                        SWM_REG.PINASSIGN[n] &= ~(0xFFu << shift);
                        SWM_REG.PINASSIGN[n] |=  (0xFFu & pConfig->PIN) << shift;
                    }
                }
                break;
