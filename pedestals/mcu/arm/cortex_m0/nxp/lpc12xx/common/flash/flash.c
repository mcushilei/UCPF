...
**
******************************************************************************/
uint32_t IAP_ErasePages(uint32_t wStartPage, uint32_t wEndPage)
{
	uint32_t wCommand[5];
	uint32_t wResult[5];
...
uint32_t flash_write_page(uint32_t wStartPage, uint32_t wSrcAddr)
{
	uint32_t wErrorCode = 0;
    uint32_t wStartSector;
    ...
    wErrorCode = IAP_CopyRAMToFlash(wStartPage << 9, wSrcAddr, FLASH_PAGE_SIZE);
    return (wErrorCode);
}
...
{
	uint32_t wErrorCode = 0;
    uint32_t wStartSector, wStartPage;
    
    wStartPage   = wDstAddr / FLASH_PAGE_SIZE;
    wStartSector = wStartPage >> 3; //!< / 8
...
uint32_t flash_erase_pages(uint32_t wStartPage, uint32_t wEndPage)
{
    uint32_t wErrorCode = 0;
    uint32_t wStartSector, wEndSector;
    
    wStartSector = wStartPage >> 3; //!< / 8
    wEndSector   = wStartPage >> 3;
    ...
    wErrorCode = IAP_ErasePages(wStartPage, wEndPage);
    return (wErrorCode);
}
